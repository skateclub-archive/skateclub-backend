...
function handleAllRequests(req, res, next) {
  ...
  const validations = [
    validate.accessControlCheck(req),
  ...
  if (failed) {
    loggerInstance.warn(failed.message);
    return res.status(failed.status).send(failed.message);
  }
...
function handlePathSpecificRequests(req, res, next, knownServers) {
    ...
    if (req.path === "/add" || req.path === "/remove") {
      ...
      if (req.path == "/add") {
        ...
        if (failed) {
          loggerInstance.warn(failed.message);
          return res.status(failed.status).send(failed.message); // Send the response message if it exists
        }
      }
      ...
      if (req.path == "/remove") {
        const validations = [
          validate.serverIPDoesNotExist(req, knownServers),
        ];
        const failed = validations.find((v) => v?.passed === false);
        if (failed) {
          loggerInstance.warn(failed.message);
